...
# D-Form Functions   
def addCombination(path, n, offset, slideRatio, angle, document):  
    # Adds the notches and stitching methods to the path 
    ...
    # for pt in points:   
        ...
        # while not(dformer.checkRelativeDifference(start_pt[0], pt[0]) and dformer.checkRelativeDifference(start_pt[1], pt[1])):
            # i += 1
            # start_pt = parsedPath[i - 1][1]
        ...
        # newPoint = dformer.computePointAlongLine(pSlope, pt, offset)
...
class Length(inkex.Effect):
    def __init__(self):
        ...
        self.OptionParser.add_option("-q", "--points",
                        action="store", type="int", 
                        dest="points", default=20,
                        help="How Many points of connection are present(teeth)")

        self.OptionParser.add_option("-n", "--offset",
                        action="store", type="float", 
                        dest="offset", default=5,
                        help="Offset")

        self.OptionParser.add_option("-y", "--angle",
                        action="store", type="float", 
                        dest="angle", default=5,
                        help="Scale Factor (Drawing:Real Length)")
                        
        self.OptionParser.add_option("-l", "--slide",
                        action="store", type="float", 
                        dest="slide", default="0",
                        help="Adjust Slide")
    ...
    def effect(self):
        ...
        addCombination(obj_nodes[id_min], self.options.points, self.options.offset, self.options.slide, self.options.angle, doc)
        addCombination(obj_nodes[id_max], self.options.points, self.options.offset, self.options.slide, self.options.angle, doc)
